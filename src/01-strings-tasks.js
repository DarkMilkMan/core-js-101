...
function getRectangleString(width, height) {
  let result = '';
  let strFirstLast = '─';
  strFirstLast = strFirstLast.repeat(width);
  let str = ' ';
  str = str.repeat(width);
  for (let i = 1; i < height + 1; i += 1) {
    if (i === 1) {
      strFirstLast = strFirstLast.replace(strFirstLast[0], '┌');
      strFirstLast = `${strFirstLast.substring(0, strFirstLast.length - 1)}┐`;
      result += `${strFirstLast}\n`;
    } else if (i === height) {
      strFirstLast = strFirstLast.replace(strFirstLast[0], '└');
      strFirstLast = `${strFirstLast.substring(0, strFirstLast.length - 1)}┘`;
...
function getCardId(value) {
  ...
  let result = 0;
  for (let i = 0; i < arrCard.length + 1; i += 1) {
    if (value === arrCard[i]) {
      result = i;
    }
  }
